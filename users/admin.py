...
from django.contrib.auth.admin import UserAdmin as BaseUserAdmin # 导入基础 UserAdmin
...
class CustomUserAdmin(BaseUserAdmin):
    ...
    if personal_info_index != -1:
        ...
        fieldsets_list[personal_info_index] = ('Personal info', {'fields': tuple(personal_info_fields)})
    else:
...
class StudentAdmin(admin.ModelAdmin):
    ...
    search_fields = ('student_id_num', 'name', 'user__username', 'user__email') # 可以搜索 Profile 字段和关联 User 的字段
    list_filter = ('grade_year', 'major', 'department', 'user__is_active') # 可以按 Profile 字段和关联 User 的字段筛选
    ...
    fieldsets = (
        ('关联用户账户', {'fields': ('user',)}), # 通常这个字段在创建后是只读或自动关联的
        ('学生基本信息', {'fields': ('student_id_num', 'name', 'id_card', 'gender', 'birth_date', 'phone')}),
        ('住宿与地址', {'fields': ('dormitory', 'home_address')}),
        ('学籍信息', {'fields': ('grade_year', 'major', 'department', 'minor_department', 'degree_level', 'credits_earned')}),
    )
...
class TeacherAdmin(admin.ModelAdmin):
    list_display = ('user', 'teacher_id_num', 'name', 'get_department_name', 'get_user_email', 'is_user_active')
    ...
    @admin.display(description='所属院系', ordering='department__dept_name') # 假设 Department 有 dept_name
