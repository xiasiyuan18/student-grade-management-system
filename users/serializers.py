...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'password': {'write_only': True, 'required': False}, # 创建时如果通过 create_user 则密码是必需的
                                                               # 更新时密码是可选的
        }
...
class StudentProfileSerializer(serializers.ModelSerializer):
    ...
    user_role = serializers.CharField(source='user.role', read_only=True) # 显示关联用户的角色
    ...
    department = serializers.PrimaryKeyRelatedField(queryset=Department.objects.all(), allow_null=True, required=False, source='department') # 明确 source
    minor_department = serializers.PrimaryKeyRelatedField(queryset=Department.objects.all(), allow_null=True, required=False)
    ...
    class Meta:
        ...
        fields = [
            ...
            'username', 'email', 'user_role', # 从关联的 CustomUser 读取
            'user_id', # 用于写入时关联 CustomUser ID

            'student_id_num', 'name', 
            'id_card', 'gender', 'birth_date', 'phone', 
            'dormitory', 'home_address', 'grade_year', 
            'major', 'department', 'minor_department', # 用于写入外键
            'major_name', 'department_name', # 用于读取外键名称
            'degree_level', 'credits_earned'
        ]
        read_only_fields = ('user', 'username', 'email', 'user_role', 'major_name', 'department_name') 
